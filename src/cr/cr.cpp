...
#pragma omp simd
  ...
  for(int k=kl; k<=ku; ++k){
    for(int j=jl; j<=ju; ++j){
      ...
      for (int i=il; i<=iu; ++i) {
        // Compute B Field Angles for Rotations
        ...
        if (va > TINY_NUMBER) {
          pcr->sigma_adv(0,k,j,i) = pcr->vmax * fabs(pcr->b_grad_pc(k,j,i)) /
...
CosmicRay::CosmicRay(MeshBlock *pmb, ParameterInput *pin) {
  ...
  pcrintegrator = std::make_unique<CRIntegrator>(this, pin);
}
...
void CosmicRay::EnrollDiffFunction(CRDiff_t MyDiffFunction) {
  UpdateDiff = MyDiffFunction;
}

// Enroll the CosmicRay Tensor Function
void CosmicRay::EnrollCRTensorFunction(CRTensor_t MyTensorFunction) {
