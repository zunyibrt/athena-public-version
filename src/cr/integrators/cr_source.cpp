                                  ...
                                  AthenaArray<Real> &u_cr) 
{
  ...
  for (int k=pmb->ks; k<=pmb->ke; ++k) {
	...
	// Compute Sigmas
        ...
        // Add Flux source term implicitly, Equation 18 (Peng & Oh)
        ...
        // Now apply the invert rotation to the new fluxes
	...
	u_cr(CRE,k,j,i) += dt*ec_source_(k,j,i);
        if (u_cr(CRE,k,j,i) < TINY_NUMBER) {
          std::cout << "foo     " << u_cr(CRE,k,j,i) << "     " << dt*ec_source_(k,j,i)  << std::endl; 
        ...
        if (u(IEN,k,j,i) < TINY_NUMBER) {
          std::cout << "bar     " << u(IEN,k,j,i) << "     " << dt*ec_source_(k,j,i)  << std::endl;
          u(IEN,k,j,i) = TINY_NUMBER;
        }
        u(IM1,k,j,i) += -(newfr1-fc1)/pcr->vmax;
        u(IM2,k,j,i) += -(newfr2-fc2)/pcr->vmax;
        u(IM3,k,j,i) += -(newfr3-fc3)/pcr->vmax;
