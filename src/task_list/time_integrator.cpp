...
TimeIntegratorTaskList::TimeIntegratorTaskList(ParameterInput *pin, Mesh *pm)
  ...
  {using namespace HydroIntegratorTaskNames; // NOLINT (build/namespace)
    ...
    // everything else
    AddTimeIntegratorTask(PHY_BVAL,CON2PRIM); // Apply Physical Boundary Condition
...
enum TaskStatus TimeIntegratorTaskList::HydroIntegrate(MeshBlock *pmb, int stage) {
  ...
  if (stage <= nstages) {
    ...
    // u = gamma_1*u + gamma_2*u1 + gamma_3*u2
    ave_wghts[0] = stage_wghts[stage-1].gamma_1;
    ave_wghts[1] = stage_wghts[stage-1].gamma_2;
    ave_wghts[2] = stage_wghts[stage-1].gamma_3;
    ph->WeightedAveU(ph->u,ph->u1,ph->u2,ave_wghts);

    // u -= beta*dt*flux_density (coordinate source terms are also handelled here)
